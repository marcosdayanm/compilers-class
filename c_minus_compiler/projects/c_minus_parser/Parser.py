...
CONCRETE_SYNTAX_TOKENS: frozenset[TokenType] = frozenset(
    {
        TokenType.SEMI,
        TokenType.COMMA,
        TokenType.LPAREN,
...
@dataclass
class ASTNode:
    """Node for the syntax tree; terminal tokens are represented as leaf nodes."""

    node_type: SyntaxNodeType | TokenType
    value: str | None = None
    ...
    def pretty_terminal_tree(self, indent: int = 0) -> str:
        if self.is_terminal:
            return " " * indent + self.terminal_label()
        if self.node_type == SyntaxNodeType.EMPTY:
            return ""

        visible_children = [
            child
            for child in self.children
            if not (not child.is_terminal and child.node_type == SyntaxNodeType.EMPTY)
        ]
        if not visible_children:
            return ""
        terminal_children = [child for child in visible_children if child.is_terminal]
        if visible_children and len(terminal_children) == len(visible_children):
            terminals = " ".join(child.terminal_label() for child in terminal_children)
            return " " * indent + f"{self.node_type.value} {terminals}"

        lines = [" " * indent + self.node_type.value]
        for child in visible_children:
            rendered = child.pretty_terminal_tree(indent + 2)
            if rendered:
                lines.append(rendered)
        return "\n".join(lines)

    def pretty_terminal_leaves(self, indent: int = 0) -> str:
        if self.is_terminal:
            if self.node_type in CONCRETE_SYNTAX_TOKENS:
                return ""
            return " " * indent + self.terminal_label()

        lines: list[str] = []
        for child in self.children:
            rendered = child.pretty_terminal_leaves(indent + 2)
            if rendered:
                lines.append(rendered)
        return "\n".join(lines)

    def terminals(self) -> list[ASTNode]:
        if self.is_terminal:
            return [self]
        terminals: list[ASTNode] = []
        for child in self.children:
            terminals.extend(child.terminals())
        return terminals

    def terminal_symbols(self) -> list[str]:
        return [terminal.node_type.name for terminal in self.terminals()]

    def abstract_terminals(self) -> list[ASTNode]:
        return [terminal for terminal in self.terminals() if terminal.node_type not in CONCRETE_SYNTAX_TOKENS]

    def abstract_terminal_symbols(self) -> list[str]:
        return [terminal.node_type.name for terminal in self.abstract_terminals()]

    def pretty_terminals(self) -> str:
        return self.pretty_terminal_leaves()

    @classmethod
    def terminal(cls, token: ParserToken) -> ASTNode:
        return cls(
...
class RecursiveDescentParser:
    ...
    def parse(self, imprime: bool = True, terminales: bool = True) -> ASTNode:
        ...
        if imprime:
            print(ast.pretty_terminals() if terminales else ast.pretty())
        return ast
    ...
    def match_node(
        ...
        context: str,
        recovery: frozenset[TokenType] = frozenset(),
    ) -> tuple[ParserToken, ASTNode]:
        token = self.match(expected, context, recovery)
        return token, ASTNode.terminal(token)
    ...
    def _var_declaration(self, fn_name="var_declaration") -> ASTNode:
        ...
        def _scalar_var_declaration() -> ASTNode:
            ...
            identifier, identifier_node = self.match_node(TokenType.ID, fn_name, frozenset({TokenType.SEMI}))
            _, semi_node = self.match_node(
                TokenType.SEMI,
                fn_name,
            ...
            return ASTNode(
                ...
                children=[type_node, identifier_node, semi_node],
        ...
        def _array_var_declaration() -> ASTNode:
            type_node = self._type_specifier()
            identifier, identifier_node = self.match_node(TokenType.ID, fn_name, frozenset({TokenType.LBRACKET}))
            _, left_bracket_node = self.match_node(TokenType.LBRACKET, fn_name, frozenset({TokenType.NUM}))
            size, size_node = self.match_node(TokenType.NUM, fn_name, frozenset({TokenType.RBRACKET}))
            _, right_bracket_node = self.match_node(TokenType.RBRACKET, fn_name, frozenset({TokenType.SEMI}))
            _, semi_node = self.match_node(
                TokenType.SEMI,
                fn_name,
                frozenset(set(DECLARATION_STARTS) | set(STATEMENT_STARTS) | {TokenType.RBRACE}),
            ...
            return ASTNode(
                ...
                children=[type_node, identifier_node, left_bracket_node, size_node, right_bracket_node, semi_node],
                line=identifier.line,
                column=identifier.column,
            )
    ...
    def _type_specifier(self) -> ASTNode:
        ...
        def _token_node(token_type: TokenType) -> ASTNode:
            token, token_node = self.match_node(token_type, "type_specifier", frozenset({TokenType.ID}))
            return ASTNode(
                SyntaxNodeType.TYPE_SPECIFIER,
                value=token.lexeme,
                children=[token_node],
                line=token.line,
                column=token.column,
    ...
    def _fun_declaration(self) -> ASTNode:
        ...
        type_node = self._type_specifier()
        identifier, identifier_node = self.match_node(TokenType.ID, "fun_declaration", frozenset({TokenType.LPAREN}))
        _, left_paren_node = self.match_node(TokenType.LPAREN, "fun_declaration", frozenset(set(TYPE_SPECIFIERS) | {TokenType.RPAREN}))
        params_node = self._params()
        _, right_paren_node = self.match_node(TokenType.RPAREN, "fun_declaration", frozenset({TokenType.LBRACE}))
        compound_node = self._compound_stmt()
        return ASTNode(
            SyntaxNodeType.FUN_DECLARATION,
            value=identifier.lexeme,
            children=[type_node, identifier_node, left_paren_node, params_node, right_paren_node, compound_node],
            line=identifier.line,
            column=identifier.column,
        )
    ...
    def _params(self) -> ASTNode:
        ...
        def void_params() -> ASTNode:
            void_token, void_node = self.match_node(TokenType.VOID, "params", frozenset({TokenType.RPAREN}))
            return ASTNode(
                SyntaxNodeType.PARAMS,
                value=void_token.lexeme,
                children=[void_node],
    ...
    def _param_list_prime(self) -> ASTNode:
        ...
        def comma_param_tail() -> ASTNode:
            _, comma_node = self.match_node(TokenType.COMMA, "param_list_prime", TYPE_SPECIFIERS)
            return ASTNode(
                SyntaxNodeType.PARAM_LIST_PRIME,
                children=[comma_node, self._param(), self._param_list_prime()],
            )
    ...
    def _param(self) -> ASTNode:
        ...
        def _scalar_param() -> ASTNode:
            type_node = self._type_specifier()
            identifier, identifier_node = self.match_node(TokenType.ID, "param", frozenset({TokenType.COMMA, TokenType.RPAREN}))
            return ASTNode(
                SyntaxNodeType.PARAM,
                value=identifier.lexeme,
                children=[type_node, identifier_node],
                line=identifier.line,
                column=identifier.column,
            )
        ...
        def _array_param() -> ASTNode:
            type_node = self._type_specifier()
            identifier, identifier_node = self.match_node(TokenType.ID, "param", frozenset({TokenType.LBRACKET}))
            _, left_bracket_node = self.match_node(TokenType.LBRACKET, "param", frozenset({TokenType.RBRACKET}))
            _, right_bracket_node = self.match_node(TokenType.RBRACKET, "param", frozenset({TokenType.COMMA, TokenType.RPAREN}))
            return ASTNode(
                SyntaxNodeType.PARAM,
                value=f"{identifier.lexeme}[]",
                children=[type_node, identifier_node, left_bracket_node, right_bracket_node],
                line=identifier.line,
                column=identifier.column,
            )
    ...
    def _compound_stmt(self) -> ASTNode:
        ...
        left, left_brace_node = self.match_node(
            TokenType.LBRACE,
            "compound_stmt",
            frozenset(set(DECLARATION_STARTS) | set(STATEMENT_STARTS) | {TokenType.RBRACE}),
        )
        ...
        _, right_brace_node = self.match_node(
            TokenType.RBRACE,
            "compound_stmt",
            frozenset(set(DECLARATION_STARTS) | set(STATEMENT_STARTS) | {TokenType.ENDFILE}),
        )
        return ASTNode(
            SyntaxNodeType.COMPOUND_STMT,
            children=[left_brace_node, local_node, statement_node, right_brace_node],
            line=left.line,
            column=left.column,
        )
    ...
    def _expression_stmt(self, fn_name="expression_stmt") -> ASTNode:
        ...
        def expression_semicolon() -> ASTNode:
            ...
            _, semi_node = self.match_node(TokenType.SEMI, fn_name, frozenset(set(STATEMENT_STARTS) | {TokenType.RBRACE}))
            return ASTNode(SyntaxNodeType.EXPRESSION_STMT, children=[expression_node, semi_node])

        def semicolon_only() -> ASTNode:
            semi, semi_node = self.match_node(TokenType.SEMI, fn_name, STATEMENT_STARTS | {TokenType.RBRACE})
            return ASTNode(SyntaxNodeType.EXPRESSION_STMT, value=semi.lexeme, children=[semi_node], line=semi.line, column=semi.column)
    ...
    def _selection_stmt(self, fn_name="selection_stmt") -> ASTNode:
        ...
        def _if_else_statement() -> ASTNode:
            if_token, if_node = self.match_node(TokenType.IF, fn_name, frozenset({TokenType.LPAREN}))
            _, left_paren_node = self.match_node(TokenType.LPAREN, fn_name, EXPRESSION_STARTS)
            condition = self._expression()
            _, right_paren_node = self.match_node(TokenType.RPAREN, fn_name, STATEMENT_STARTS)
            then_statement = self._statement(fn_name)
            _, else_node = self.match_node(TokenType.ELSE, fn_name, STATEMENT_STARTS)
            else_statement = self._statement(fn_name)
            return ASTNode(
                SyntaxNodeType.SELECTION_STMT,
                children=[if_node, left_paren_node, condition, right_paren_node, then_statement, else_node, else_statement],
                line=if_token.line,
        ...
        def _if_statement() -> ASTNode:
            ...
            return ASTNode(
                SyntaxNodeType.SELECTION_STMT,
                children=[if_node, left_paren_node, condition, right_paren_node, then_statement],
                line=if_token.line,
                column=if_token.column,
            )
    ...
    def _iteration_stmt(self, fn_name="iteration_stmt") -> ASTNode:
        ...
        while_token, while_node = self.match_node(TokenType.WHILE, fn_name, frozenset({TokenType.LPAREN}))
        _, left_paren_node = self.match_node(TokenType.LPAREN, fn_name, EXPRESSION_STARTS)
        condition = self._expression()
        _, right_paren_node = self.match_node(TokenType.RPAREN, fn_name, STATEMENT_STARTS)
        body = self._statement(fn_name)
        return ASTNode(
            SyntaxNodeType.ITERATION_STMT,
            children=[while_node, left_paren_node, condition, right_paren_node, body],
    ...
    def _return_stmt(self, fn_name="return_stmt") -> ASTNode:
        ...
        def return_expression() -> ASTNode:
            return_token, return_node = self.match_node(TokenType.RETURN, fn_name, frozenset(set(EXPRESSION_STARTS) | {TokenType.SEMI}))
            expression_node = self._expression()
            _, semi_node = self.match_node(TokenType.SEMI, fn_name, frozenset(set(STATEMENT_STARTS) | {TokenType.RBRACE}))
            return ASTNode(
                SyntaxNodeType.RETURN_STMT,
                children=[return_node, expression_node, semi_node],
                line=return_token.line,
                column=return_token.column,
            )

        def return_semicolon() -> ASTNode:
            return_token, return_node = self.match_node(TokenType.RETURN, fn_name, frozenset({TokenType.SEMI}))
            _, semi_node = self.match_node(TokenType.SEMI, fn_name, STATEMENT_STARTS | {TokenType.RBRACE})
            return ASTNode(
                SyntaxNodeType.RETURN_STMT,
                children=[return_node, semi_node],
                line=return_token.line,
                column=return_token.column,
            )
    ...
    def _expression(self, fn_name="expression") -> ASTNode:
        ...
        def assignment_expression() -> ASTNode:
            var_node = self._var()
            assign_token, assign_node = self.match_node(TokenType.ASSIGN, fn_name, EXPRESSION_STARTS)
            expression_node = self._expression(fn_name)
            return ASTNode(
                SyntaxNodeType.EXPRESSION,
                value=assign_token.lexeme,
                children=[var_node, assign_node, expression_node],
    ...
    def _var(self, fn_name="var") -> ASTNode:
        ...
        def scalar_var() -> ASTNode:
            identifier, identifier_node = self.match_node(
                TokenType.ID,
                fn_name,
        ...
        def array_var() -> ASTNode:
            identifier, identifier_node = self.match_node(TokenType.ID, fn_name, frozenset({TokenType.LBRACKET}))
            _, left_bracket_node = self.match_node(TokenType.LBRACKET, fn_name, EXPRESSION_STARTS)
            index = self._expression(fn_name)
            _, right_bracket_node = self.match_node(
                TokenType.RBRACKET,
                fn_name,
                frozenset(set(ADDOPS) | set(MULOPS) | set(RELOPS) | {TokenType.ASSIGN, TokenType.SEMI, TokenType.COMMA, TokenType.RPAREN, TokenType.RBRACKET}),
            )
            return ASTNode(
                SyntaxNodeType.VAR,
                value=f"{identifier.lexeme}[]",
                children=[identifier_node, left_bracket_node, index, right_bracket_node],
                line=identifier.line,
                column=identifier.column,
            )
    ...
    def _factor(self) -> ASTNode:
        ...
        def grouped_expression() -> ASTNode:
            left, left_paren_node = self.match_node(TokenType.LPAREN, "factor", EXPRESSION_STARTS)
            expression_node = self._expression()
            _, right_paren_node = self.match_node(
                TokenType.RPAREN,
                "factor",
                frozenset(set(ADDOPS) | set(MULOPS) | set(RELOPS) | {TokenType.SEMI, TokenType.COMMA, TokenType.RBRACKET, TokenType.RPAREN}),
            )
            return ASTNode(
                SyntaxNodeType.FACTOR,
                value="()",
                children=[left_paren_node, expression_node, right_paren_node],
                line=left.line,
                column=left.column,
            )

        def number_factor() -> ASTNode:
            number, number_node = self.match_node(TokenType.NUM, "factor")
            return ASTNode(
                SyntaxNodeType.FACTOR,
                value=number.lexeme,
    ...
    def _call(self) -> ASTNode:
        ...
        identifier, identifier_node = self.match_node(TokenType.ID, "call", frozenset({TokenType.LPAREN}))
        _, left_paren_node = self.match_node(TokenType.LPAREN, "call", frozenset(set(EXPRESSION_STARTS) | {TokenType.RPAREN}))
        ...
        return ASTNode(
            SyntaxNodeType.CALL,
            value=identifier.lexeme,
            children=[identifier_node, left_paren_node, args_node, right_paren_node],
            line=identifier.line,
            column=identifier.column,
        )
    ...
    def _arg_list_prime(self) -> ASTNode:
        ...
        def comma_expression_tail() -> ASTNode:
            _, comma_node = self.match_node(TokenType.COMMA, "arg_list_prime", EXPRESSION_STARTS)
            return ASTNode(
                SyntaxNodeType.ARG_LIST_PRIME,
                children=[comma_node, self._expression(), self._arg_list_prime()],
            )
    ...
    def operator_node(self, node_type: SyntaxNodeType, token_type: TokenType, context: str) -> ASTNode:
        ...
        token, token_node = self.match_node(token_type, context, EXPRESSION_STARTS)
        return ASTNode(node_type, value=token.lexeme, children=[token_node], line=token.line, column=token.column)
...
def parser(imprime: bool = True, terminales: bool = True) -> ASTNode:
    global _lexer
    global _parser
    global _last_errors
    ...
    ast = _parser.parse(imprime, terminales)
