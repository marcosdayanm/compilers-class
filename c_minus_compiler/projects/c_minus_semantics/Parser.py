...
class RecursiveDescentParser:
    ...
    def _declaration(self, fn_name="declaration") -> ASTNode:
        ...
        declaration_kind = self._lookahead_declaration_kind()
        if declaration_kind == SyntaxNodeType.FUN_DECLARATION:
            return ASTNode(SyntaxNodeType.DECLARATION, children=[self._fun_declaration()])
        if declaration_kind == SyntaxNodeType.VAR_DECLARATION:
            return ASTNode(SyntaxNodeType.DECLARATION, children=[self._var_declaration()])
        return ASTNode(
    ...
    def _lookahead_declaration_kind(self) -> SyntaxNodeType | None:
        """Disambiguate declarations by the token after type_specifier ID."""
        if self.current_token.token_type not in TYPE_SPECIFIERS:
            return None
        ...
        self.advance()
        next_token = self.current_token.token_type
        self.reset(start)

        if next_token == TokenType.LPAREN:
            return SyntaxNodeType.FUN_DECLARATION
        if next_token in {TokenType.SEMI, TokenType.LBRACKET}:
            return SyntaxNodeType.VAR_DECLARATION
        return None
