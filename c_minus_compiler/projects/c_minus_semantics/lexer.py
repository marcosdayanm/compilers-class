...
class Lexer:
    def __init__(self, prog: str, pos: int, long: int) -> None:
        ...
        self.last_token_line: int = 1
        self.last_token_column: int = 1
    ...
    def get_token(self, print_token: bool = True) -> tuple[TokenType, str]:
        return self._get_token(print_token)

    def get_token_position(self) -> tuple[int, int]:
        return self.last_token_line, self.last_token_column

    def source_line(self, line_number: int) -> str:
        return self._line_slice(line_number)
    ...
    def _get_token(self, print_token: bool = True) -> tuple[TokenType, str]:
        while True:
            ...
            if result.token is None:
                continue
            self.last_token_line = result.line
            self.last_token_column = result.column
            if print_token:
    ...
    def _handle_id_reserved(self, context: FinalContext) -> ScanResult:
        return ScanResult(self._reserved_lookup(context.lexeme), context.lexeme, context.start_line, context.start_column)

    def _handle_int(self, context: FinalContext) -> ScanResult:
        return ScanResult(TokenType.NUM, context.lexeme, context.start_line, context.start_column)
    ...
    def _handle_num_error(self, context: FinalContext) -> ScanResult:
        ...
        return ScanResult(TokenType.ERROR, context.lexeme, context.start_line, context.start_column)

    def _handle_sign(self, context: FinalContext) -> ScanResult:
        return ScanResult(TokenType(context.lexeme), context.lexeme, context.start_line, context.start_column)
    ...
    def _handle_comment(self, context: FinalContext) -> ScanResult:
        del context
        return ScanResult(None, "", self.curr_line, self.curr_col)
    ...
    def _handle_eof(self, context: FinalContext) -> ScanResult:
        return ScanResult(TokenType.ENDFILE, "$", context.start_line, context.start_column)
...
def getToken(imprime: bool = True) -> tuple[TokenType, str]:
    ...
    return _lexer.get_token(imprime)


def getTokenPosition() -> tuple[int, int]:
    if _lexer is None:
        raise RuntimeError("The lexer has not been initialized. Call globales() first.")
    return _lexer.get_token_position()


def getSourceLine(line_number: int) -> str:
    if _lexer is None:
        raise RuntimeError("The lexer has not been initialized. Call globales() first.")
    return _lexer.source_line(line_number)
